...
type DataFarinhaDeTrigo50kg struct {
...
type FarinhaDeTrigo50Kg struct {
	Atualizacao time.Time                `json:"data_atualizacao"`
	Fonte       string                   `json:"fonte"`
	Data        []DataFarinhaDeTrigo50kg `json:"data"`
}

func RunnerFarinhaDeTrigo() {
	runnerName := "Preços - FarinhaDeTrigo 50KG"
	domain := "www.ipeadata.gov.br"
	url := "http://www.ipeadata.gov.br/ExibeSerie.aspx?serid=37611&module=M"
	file_path := "./data/precos/farinhadetrigo-50kg.json"
	fileNameOutputCSV := "./data/precos/farinhadetrigo-50kg.csv"
	s3KeyJSON := "precos/farinhadetrigo-50kg.json"
	s3KeyCSV := "precos/farinhadetrigo-50kg.csv"
	...
	farinhadetrigo := &FarinhaDeTrigo50Kg{}
	...
	now := time.Now()
	farinhadetrigo.Atualizacao = now
	farinhadetrigo.Fonte = url
	...
	c.OnHTML(".dxgvTable", func(e *colly.HTMLElement) {
		...
		e.ForEach("tr", func(i int, tr *colly.HTMLElement) {
			...
			farinhadetrigo.Data = append(farinhadetrigo.Data, item)
		...
		b, err := json.Marshal(farinhadetrigo)
		if err != nil {
			l.Fatal().
				Str("Runner", runnerName).
				Str("Error", err.Error()).
		...
		csvOutput, err := gocsv.MarshalString(&farinhadetrigo.Data)
		if err != nil {
			l.Fatal().
				Str("Runner", runnerName).
				Str("FilePath", fileNameOutputCSV).
