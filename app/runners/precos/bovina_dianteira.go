...
type DataBovinaDianteirakg struct {
...
type BovinaDianteiraKg struct {
	Atualizacao time.Time               `json:"data_atualizacao"`
	Fonte       string                  `json:"fonte"`
	Data        []DataBovinaDianteirakg `json:"data"`
}

func RunnerBovinaDianteira() {
	runnerName := "Preços - Carne Bovina Dianteira Kg"
	domain := "www.ipeadata.gov.br"
	url := "http://www.ipeadata.gov.br/ExibeSerie.aspx?serid=37601&module=M"
	file_path := "./data/precos/carne-bovina-dianteira.json"
	fileNameOutputCSV := "./data/precos/carne-bovina-dianteira.csv"
	s3KeyJSON := "precos/carne-bovina-dianteira.json"
	s3KeyCSV := "precos/carne-bovina-dianteira.csv"
