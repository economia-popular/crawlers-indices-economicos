...
type DataOvoGrandeTipo230duzias struct {
...
type OvoGrandeTipo230Duzias struct {
	Atualizacao time.Time                    `json:"data_atualizacao"`
	Fonte       string                       `json:"fonte"`
	Data        []DataOvoGrandeTipo230duzias `json:"data"`
}

func RunnerOvoGrandeTipo2() {
	runnerName := "Preços - OvoGrandeTipo2 30 Duzias"
	domain := "www.ipeadata.gov.br"
	url := "http://www.ipeadata.gov.br/ExibeSerie.aspx?serid=37627&module=M"
	file_path := "./data/precos/ovograndetipo2-30duzias.json"
	fileNameOutputCSV := "./data/precos/ovograndetipo2-30duzias.csv"
	s3KeyJSON := "precos/ovograndetipo2-30duzias.json"
	s3KeyCSV := "precos/ovograndetipo2-30duzias.csv"
	...
	ovograndetipo2 := &OvoGrandeTipo230Duzias{}
	...
	now := time.Now()
	ovograndetipo2.Atualizacao = now
	ovograndetipo2.Fonte = url
	...
	c.OnHTML(".dxgvTable", func(e *colly.HTMLElement) {
		...
		e.ForEach("tr", func(i int, tr *colly.HTMLElement) {
			...
			item := DataOvoGrandeTipo230duzias{
			...
			ovograndetipo2.Data = append(ovograndetipo2.Data, item)
		...
		b, err := json.Marshal(ovograndetipo2)
		if err != nil {
			l.Fatal().
				Str("Runner", runnerName).
				Str("Error", err.Error()).
		...
		csvOutput, err := gocsv.MarshalString(&ovograndetipo2.Data)
		if err != nil {
			l.Fatal().
				Str("Runner", runnerName).
				Str("FilePath", fileNameOutputCSV).
