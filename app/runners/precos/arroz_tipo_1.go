...
type DataArrozTipo130kg struct {
...
type ArrozTipo130Kg struct {
	Atualizacao time.Time            `json:"data_atualizacao"`
	Fonte       string               `json:"fonte"`
	Data        []DataArrozTipo130kg `json:"data"`
}

func RunnerArrozTipo1() {
	runnerName := "Preços - ArrozTipo1 30KG"
	domain := "www.ipeadata.gov.br"
	url := "http://www.ipeadata.gov.br/ExibeSerie.aspx?serid=37617&module=M"
	file_path := "./data/precos/arroztipo1-30kg.json"
	fileNameOutputCSV := "./data/precos/arroztipo1-30kg.csv"
	s3KeyJSON := "precos/arroztipo1-30kg.json"
	s3KeyCSV := "precos/arroztipo1-30kg.csv"
	...
	now := time.Now()
	arroztipo1.Atualizacao = now
	arroztipo1.Fonte = url
	...
	c.OnHTML(".dxgvTable", func(e *colly.HTMLElement) {
		...
		e.ForEach("tr", func(i int, tr *colly.HTMLElement) {
			...
			arroztipo1.Data = append(arroztipo1.Data, item)
		...
		csvOutput, err := gocsv.MarshalString(&arroztipo1.Data)
		if err != nil {
			l.Fatal().
				Str("Runner", runnerName).
				Str("FilePath", fileNameOutputCSV).
