...
type DataOleoDeSoja20Latas struct {
	...
	QtdLatas   float64 `json:"qtd_latas" csv:"qtd_latas"`
}

type OleoDeSoja50Kg struct {
	Atualizacao time.Time               `json:"data_atualizacao"`
	Fonte       string                  `json:"fonte"`
	Data        []DataOleoDeSoja20Latas `json:"data"`
}

func RunnerOleoDeSoja() {
	runnerName := "Preços - OleoDeSoja 50KG"
	domain := "www.ipeadata.gov.br"
	url := "http://www.ipeadata.gov.br/ExibeSerie.aspx?serid=37603&module=M"
	file_path := "./data/precos/oleodesoja-20latas.json"
	fileNameOutputCSV := "./data/precos/oleodesoja-20latas.csv"
	s3KeyJSON := "precos/oleodesoja-20latas.json"
	s3KeyCSV := "precos/oleodesoja-20latas.csv"
	...
	now := time.Now()
	oleodesoja.Atualizacao = now
	oleodesoja.Fonte = url
	...
	c.OnHTML(".dxgvTable", func(e *colly.HTMLElement) {
		...
		e.ForEach("tr", func(i int, tr *colly.HTMLElement) {
			...
			oleodesoja.Data = append(oleodesoja.Data, item)
		...
		csvOutput, err := gocsv.MarshalString(&oleodesoja.Data)
		if err != nil {
			l.Fatal().
				Str("Runner", runnerName).
				Str("FilePath", fileNameOutputCSV).
