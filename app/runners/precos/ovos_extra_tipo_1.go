...
type DataOvoExtraTipo130duzias struct {
...
type OvoExtraTipo130Duzias struct {
	Atualizacao time.Time                   `json:"data_atualizacao"`
	Fonte       string                      `json:"fonte"`
	Data        []DataOvoExtraTipo130duzias `json:"data"`
}

func RunnerOvoExtraTipo1() {
	runnerName := "Preços - OvoExtraTipo1 30KG"
	domain := "www.ipeadata.gov.br"
	url := "http://www.ipeadata.gov.br/ExibeSerie.aspx?serid=37632&module=M"
	file_path := "./data/precos/ovoextratipo1-30duzias.json"
	fileNameOutputCSV := "./data/precos/ovoextratipo1-30duzias.csv"
	s3KeyJSON := "precos/ovoextratipo1-30duzias.json"
	s3KeyCSV := "precos/ovoextratipo1-30duzias.csv"
	...
	ovoextratipo1 := &OvoExtraTipo130Duzias{}
	...
	now := time.Now()
	ovoextratipo1.Atualizacao = now
	ovoextratipo1.Fonte = url
	...
	c.OnHTML(".dxgvTable", func(e *colly.HTMLElement) {
		...
		e.ForEach("tr", func(i int, tr *colly.HTMLElement) {
			...
			item := DataOvoExtraTipo130duzias{
			...
			ovoextratipo1.Data = append(ovoextratipo1.Data, item)
		...
		b, err := json.Marshal(ovoextratipo1)
		if err != nil {
			l.Fatal().
				Str("Runner", runnerName).
				Str("Error", err.Error()).
		...
		csvOutput, err := gocsv.MarshalString(&ovoextratipo1.Data)
		if err != nil {
			l.Fatal().
				Str("Runner", runnerName).
				Str("FilePath", fileNameOutputCSV).
