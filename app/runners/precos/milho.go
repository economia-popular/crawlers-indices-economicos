...
type Milho60Kg struct {
	Atualizacao time.Time       `json:"data_atualizacao"`
	Fonte       string          `json:"fonte"`
	Data        []DataMilho60kg `json:"data"`
}

func RunnerMilho() {
	runnerName := "Preços - Milho 60KG"
	domain := "www.ipeadata.gov.br"
	url := "http://www.ipeadata.gov.br/ExibeSerie.aspx?serid=37649&module=M"
	file_path := "./data/precos/milho-60kg.json"
	fileNameOutputCSV := "./data/precos/milho-60kg.csv"
	s3KeyJSON := "precos/milho-60kg.json"
	s3KeyCSV := "precos/milho-60kg.csv"
