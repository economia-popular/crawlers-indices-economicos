...
type SuinaPernilKg struct {
	Atualizacao time.Time           `json:"data_atualizacao"`
	Fonte       string              `json:"fonte"`
	Data        []DataSuinaPernilkg `json:"data"`
}

func RunnerSuinaPernil() {
	runnerName := "Preços - Carne Suina Pernil Kg"
	domain := "www.ipeadata.gov.br"
	url := "http://www.ipeadata.gov.br/ExibeSerie.aspx?serid=37607&module=M"
	file_path := "./data/precos/carne-suina-pernil.json"
	fileNameOutputCSV := "./data/precos/carne-suina-pernil.csv"
	s3KeyJSON := "precos/carne-suina-pernil.json"
	s3KeyCSV := "precos/carne-suina-pernil.csv"
