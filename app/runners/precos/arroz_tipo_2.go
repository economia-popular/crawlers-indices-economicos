...
type DataArrozTipo230kg struct {
...
type ArrozTipo230Kg struct {
	Atualizacao time.Time            `json:"data_atualizacao"`
	Fonte       string               `json:"fonte"`
	Data        []DataArrozTipo230kg `json:"data"`
}

func RunnerArrozTipo2() {
	runnerName := "Preços - ArrozTipo2 30KG"
	domain := "www.ipeadata.gov.br"
	url := "http://www.ipeadata.gov.br/ExibeSerie.aspx?serid=37600&module=M"
	file_path := "./data/precos/arroztipo2-30kg.json"
	fileNameOutputCSV := "./data/precos/arroztipo2-30kg.csv"
	s3KeyJSON := "precos/arroztipo2-30kg.json"
	s3KeyCSV := "precos/arroztipo2-30kg.csv"
	...
	now := time.Now()
	arroztipo2.Atualizacao = now
	arroztipo2.Fonte = url
	...
	c.OnHTML(".dxgvTable", func(e *colly.HTMLElement) {
		...
		e.ForEach("tr", func(i int, tr *colly.HTMLElement) {
			...
			arroztipo2.Data = append(arroztipo2.Data, item)
		...
		csvOutput, err := gocsv.MarshalString(&arroztipo2.Data)
		if err != nil {
			l.Fatal().
				Str("Runner", runnerName).
				Str("FilePath", fileNameOutputCSV).
