...
type DataBovinaTraseirakg struct {
...
type BovinaTraseiraKg struct {
	Atualizacao time.Time              `json:"data_atualizacao"`
	Fonte       string                 `json:"fonte"`
	Data        []DataBovinaTraseirakg `json:"data"`
}

func RunnerBovinaTraseira() {
	runnerName := "Preços - Carne Bovina Traseira Kg"
	domain := "www.ipeadata.gov.br"
	url := "http://www.ipeadata.gov.br/ExibeSerie.aspx?serid=37604&module=M"
	file_path := "./data/precos/carne-bovina-traseira.json"
	fileNameOutputCSV := "./data/precos/carne-bovina-traseira.csv"
	s3KeyJSON := "precos/carne-bovina-traseira.json"
	s3KeyCSV := "precos/carne-bovina-traseira.csv"
