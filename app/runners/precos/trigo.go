...
type Trigo60Kg struct {
	Atualizacao time.Time       `json:"data_atualizacao"`
	Fonte       string          `json:"fonte"`
	Data        []DataTrigo60kg `json:"data"`
}

func RunnerTrigo() {
	runnerName := "Preços - Trigo 60KG"
	domain := "www.ipeadata.gov.br"
	url := "http://www.ipeadata.gov.br/ExibeSerie.aspx?serid=37648&module=M"
	file_path := "./data/precos/trigo-60kg.json"
	fileNameOutputCSV := "./data/precos/trigo-60kg.csv"
	s3KeyJSON := "precos/trigo-60kg.json"
	s3KeyCSV := "precos/trigo-60kg.csv"
	...
	c.OnHTML(".dxgvTable", func(e *colly.HTMLElement) {
		...
		e.ForEach("tr", func(i int, tr *colly.HTMLElement) {
			...
			trigo.Data = append(trigo.Data, item)
		...
		csvOutput, err := gocsv.MarshalString(&trigo.Data)
		if err != nil {
			l.Fatal().
				Str("Runner", runnerName).
				Str("FilePath", fileNameOutputCSV).
