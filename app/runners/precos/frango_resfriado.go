...
type DataFrangoResfriado struct {
...
type FrangoResfriado struct {
	Atualizacao time.Time             `json:"data_atualizacao"`
	Fonte       string                `json:"fonte"`
	Data        []DataFrangoResfriado `json:"data"`
}

func RunnerFrangoResfriado() {
	runnerName := "Preços - FrangoResfriado Kg"
	domain := "www.ipeadata.gov.br"
	url := "http://www.ipeadata.gov.br/ExibeSerie.aspx?serid=37638&module=M"
	file_path := "./data/precos/frango-resfriado.json"
	fileNameOutputCSV := "./data/precos/frango-resfriado.csv"
	s3KeyJSON := "precos/frango-resfriado.json"
	s3KeyCSV := "precos/frango-resfriado.csv"
