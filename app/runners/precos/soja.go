...
type DataSojaTonelada struct {
	...
	Tonelada   float64 `json:"tonelada" csv:"tonelada"`
}

type SojaTonelada struct {
	Atualizacao time.Time          `json:"data_atualizacao"`
	Fonte       string             `json:"fonte"`
	Data        []DataSojaTonelada `json:"data"`
}

func RunnerSoja() {
	runnerName := "Preços - Soja"
	domain := "www.ipeadata.gov.br"
	url := "http://www.ipeadata.gov.br/ExibeSerie.aspx?serid=37618&module=M"
	file_path := "./data/precos/soja-tonelada.json"
	fileNameOutputCSV := "./data/precos/soja-tonelada.csv"
	s3KeyJSON := "precos/soja-tonelada.json"
	s3KeyCSV := "precos/soja-tonelada.csv"
	...
	c.OnHTML(".dxgvTable", func(e *colly.HTMLElement) {
		...
		e.ForEach("tr", func(i int, tr *colly.HTMLElement) {
			...
			soja.Data = append(soja.Data, item)
		...
		csvOutput, err := gocsv.MarshalString(&soja.Data)
		if err != nil {
			l.Fatal().
				Str("Runner", runnerName).
				Str("FilePath", fileNameOutputCSV).
