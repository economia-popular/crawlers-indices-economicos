...
type SuinaLomboKg struct {
	Atualizacao time.Time          `json:"data_atualizacao"`
	Fonte       string             `json:"fonte"`
	Data        []DataSuinaLombokg `json:"data"`
}

func RunnerSuinaLombo() {
	runnerName := "Preços - Carne Suina Lombo Kg"
	domain := "www.ipeadata.gov.br"
	url := "http://www.ipeadata.gov.br/ExibeSerie.aspx?serid=37612&module=M"
	file_path := "./data/precos/carne-suina-lombo.json"
	fileNameOutputCSV := "./data/precos/carne-suina-lombo.csv"
	s3KeyJSON := "precos/carne-suina-lombo.json"
	s3KeyCSV := "precos/carne-suina-lombo.csv"
