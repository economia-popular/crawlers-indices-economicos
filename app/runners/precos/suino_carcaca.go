...
type DataSuinaCarcacakg struct {
...
type SuinaCarcacaKg struct {
	Atualizacao time.Time            `json:"data_atualizacao"`
	Fonte       string               `json:"fonte"`
	Data        []DataSuinaCarcacakg `json:"data"`
}

func RunnerSuinaCarcaca() {
	runnerName := "Preços - Carne Suina Carcaca Kg"
	domain := "www.ipeadata.gov.br"
	url := "http://www.ipeadata.gov.br/ExibeSerie.aspx?serid=37626&module=M"
	file_path := "./data/precos/carne-suina-carcaca.json"
	fileNameOutputCSV := "./data/precos/carne-suina-carcaca.csv"
	s3KeyJSON := "precos/carne-suina-carcaca.json"
	s3KeyCSV := "precos/carne-suina-carcaca.csv"
