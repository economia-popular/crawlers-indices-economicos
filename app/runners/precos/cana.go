...
type Cana struct {
	...
	Data        []DataCana `json:"data"`
}

func RunnerCana() {
	runnerName := "Preços - Cana Kg"
	domain := "www.ipeadata.gov.br"
	url := "http://www.ipeadata.gov.br/ExibeSerie.aspx?serid=37622&module=M"
	file_path := "./data/precos/cana.json"
	fileNameOutputCSV := "./data/precos/cana.csv"
	s3KeyJSON := "precos/cana.json"
	s3KeyCSV := "precos/cana.csv"
